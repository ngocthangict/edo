...
class Widget_Edo_Custom_Image extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
                        ...
                        'description' => __( 'You can add custom image.', 'edo' ) );
		parent::__construct( 'widget_edo_custom_image', __('Edo Custom Image', 'edo' ), $widget_ops );
	}
            ...
            <?php 
             if(isset($instance[ 'title' ]) && $instance[ 'title' ] && count($instance[ 'title' ]) > 0 ):
                for( $i = 0; $i < count($instance['title']); $i++ ):
                    ...
                    ?>
                    <li>
                        <a target="<?php echo esc_attr( $target ) ?>" href="<?php echo esc_attr($link) ?>"><img src="<?php echo $img_preview; ?>" alt="<?php echo esc_attr($title) ?>" /><span class="title"><?php echo esc_attr($title) ?></span></a>
                    </li>
                    <?php endif; ?>
                <?php endfor; ?>
            <?php endif; ?>
	...
	?>
        <p>
            <label for="<?php echo $this->get_field_id( 'wtitle' ); ?>"><?php _e( 'Title:', 'edo'); ?></label> 
                        ...
                        <div class="item widget-content">
                            ...
                            <p>
                    			...
                    			<select name="<?php echo $this->get_field_name('target'); ?>[]" id="<?php echo $this->get_field_id('target'); ?>" class="widefat">
                    				<option value="_blank"<?php selected( $target, '_blank' ); ?>><?php _e('Open New Window', 'edo'); ?></option>
                    				<option value="_self"<?php selected( $target, '_self' ); ?>><?php _e('Stay in Window', 'edo'); ?></option>
                    			</select>
                    		</p>
                        </div>
                    <?php }}
