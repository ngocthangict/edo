 ...
 */
function edo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'edo_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'edo' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => edo_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'edo' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'edo' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'edo' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'edo' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'edo' );
}
add_action( 'customize_register', 'edo_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'edo_color_schemes'}.
 *
 ...
 */
function edo_get_color_schemes() {
	return apply_filters( 'edo_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'edo' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'edo_get_color_scheme' ) ) :
/**
 ...
 */
function edo_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = edo_get_color_schemes();
...
endif; // edo_get_color_scheme

if ( ! function_exists( 'edo_get_color_scheme_choices' ) ) :
/**
 ...
 */
function edo_get_color_scheme_choices() {
	$color_schemes                = edo_get_color_schemes();
...
endif; // edo_get_color_scheme_choices

if ( ! function_exists( 'edo_sanitize_color_scheme' ) ) :
/**
 ...
 */
function edo_sanitize_color_scheme( $value ) {
	$color_schemes = edo_get_color_scheme_choices();
...
endif; // edo_sanitize_color_scheme
...
function edo_color_scheme_css() {
	...
	$color_textcolor_rgb         = edo_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = edo_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = edo_get_color_scheme_css( $colors );

	wp_add_inline_style( 'edo-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'edo_color_scheme_css' );
 ...
 * @since edo 1.0
 */
function edo_customize_control_js() {
	wp_enqueue_script( 'color-scheme-control', THEME_URL . '/js/color-scheme-control.js', array( 'customize-controls', 'iris', 'underscore', 'wp-util' ), '20141216', true );
	wp_localize_script( 'color-scheme-control', 'colorScheme', edo_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'edo_customize_control_js' );
 ...
 * @since edo 1.0
 */
function edo_customize_preview_js() {
	wp_enqueue_script( 'edo-customize-preview', THEME_URL . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'edo_customize_preview_js' );
 ...
 */
function edo_get_color_scheme_css( $colors ) {
 ...
 * @since edo 1.0
 */
function edo_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-edo-color-scheme">
		<?php echo edo_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'edo_color_scheme_css_template' );
